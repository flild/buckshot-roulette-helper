    ...
    <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in-95 duration-300 relative bg-[radial-gradient(circle_at_center,_#0f0f0f_0%,_#050505_100%)]">
      ...
      <div className="w-full max-w-4xl text-center space-y-16 px-4 py-12">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-12 sm:gap-24">
          <div>
            <div className="text-[12px] text-[#ff2e2e] uppercase tracking-[0.3em] mb-4 font-bold">{t('assist.liveLeft')}</div>
          ...
          <div>
            <div className="text-[12px] text-[#888] uppercase tracking-[0.3em] mb-4 font-bold">{t('assist.blankLeft')}</div>
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 justify-center w-full">
          {!isEmpty ? (
            <>
              ...
              >
                {t('assist.liveBtn')} <span className="text-lg opacity-50 group-hover:text-black block transition-colors">[L]</span>
              </button>
              <button
              ...
              >
                {t('assist.blankBtn')} <span className="text-lg opacity-50 group-hover:text-black block transition-colors">[B]</span>
              </button>
            </>
          ) : (
            <div className="w-full flex flex-col sm:flex-row gap-4 justify-center">
               <button onClick={() => endRound('abandoned')} className="w-full sm:w-auto px-12 py-8 bg-[#111] border-4 border-[#e0e0e0] text-[#e0e0e0] hover:text-black hover:shadow-[0_0_40px_rgba(224,224,224,0.4)] text-xl sm:text-2xl font-black uppercase hover:bg-[#e0e0e0] transition-all duration-300 active:scale-95">{t('assist.nextMag')}</button>
               <button onClick={() => endRound('win')} className="w-full sm:w-auto px-12 py-8 bg-[#1a1a05] border-4 border-[#ffcc00] text-[#ffcc00] hover:text-black hover:shadow-[0_0_50px_rgba(255,204,0,0.5)] text-xl sm:text-2xl font-black uppercase hover:bg-[#ffcc00] transition-all duration-300 active:scale-95">{t('assist.winRound')}</button>
               <button onClick={() => endRound('loss')} className="w-full sm:w-auto px-12 py-8 bg-[#1a0505] border-4 border-[#ff2e2e] text-[#ff2e2e] hover:text-black hover:shadow-[0_0_50px_rgba(255,46,46,0.6)] text-xl sm:text-2xl font-black uppercase hover:bg-[#ff2e2e] transition-all duration-300 active:scale-95">{t('assist.loss')}</button>
            </div>
          )}
        </div>
